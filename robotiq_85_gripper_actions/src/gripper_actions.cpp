...
void GripperActions::executeGripperCommand(const control_msgs::GripperCommandGoalConstPtr &goal)
{
    ...
    cmd.stop = false;
    cmd.position = (-0.10897)*(goal->command.position)+0.085;
    ...
    ros::Duration(0.5).sleep(); //give gripper time to start moving default=0.25
    while (!(positionReached || gripperStopped))
    {
        gripperStopped = !gripperStatus.is_moving;
        ROS_INFO("Gripper Stop");
        if (fabs(gripperStatus.position - cmd.position) < .001)
            {
